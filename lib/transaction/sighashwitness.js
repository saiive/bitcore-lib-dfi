...
var sighash = function sighash(transaction, sighashType, inputNumber, scriptCode, satoshisBuffer) {
  ...
  if ((sighashType & 0x1f) !== Signature.SIGHASH_SINGLE && (sighashType & 0x1f) !== Signature.SIGHASH_NONE) {
    for (var p = 0; p < transaction.outputs.length; p++) {
      transaction.outputs[p].toBufferWriter(outputWriter, transaction.version);
    }
    ...
    transaction.outputs[inputNumber].toBufferWriter(outputWriter, transaction.version);
    hashOutputs = Hash.sha256sha256(outputWriter.toBuffer());
  }
...
/**
 * Create a signature with WithInterface
 *
 * @name Signing.signWithInterface
 * @param {Transaction} transaction
 * @param {Function} signInterface
 * @param {number} sighash
 * @param {number} inputIndex
 * @param {Script} subscript
 * @param {String} signingMethod - method used to sign - 'ecdsa' or 'schnorr'
 * @return {Signature}
 */
async function signWithInterface(transaction, signInterface, sighashType, inputIndex, scriptCode, satoshisBuffer, signingMethod) {
  signingMethod = signingMethod || 'ecdsa';
  if (signingMethod === 'ecdsa') {
    const hashbuf = sighash(transaction, sighashType, inputIndex, scriptCode, satoshisBuffer);
    const sig = await signInterface(hashbuf)
    return sig;
  }
  throw new Error("signingMethod not supported ", signingMethod);
}
...
module.exports = {
  sighash: sighash,
  sign: sign,
  signWithInterface: signWithInterface,
  verify: verify
};
