...
/**
 * Create a signature with ledger
 *
 * @name Signing.signLedger
 * @param {Transaction} transaction
 * @param {Function} getSignLedger
 * @param {number} sighash
 * @param {number} inputIndex
 * @param {Script} subscript
 * @param {String} signingMethod - method used to sign - 'ecdsa' or 'schnorr'
 * @return {Signature}
 */
async function signLedger(transaction, getSignLedger, sighashType, inputIndex, scriptCode, satoshisBuffer, signingMethod) {
  signingMethod = signingMethod || 'ecdsa';
  if (signingMethod === 'ecdsa') {
    const hashbuf = sighash(transaction, sighashType, inputIndex, scriptCode, satoshisBuffer);
    const sig = await getSignLedger(hashbuf)
    return sig;
  }
  throw new Error("signingMethod not supported ", signingMethod);
}
