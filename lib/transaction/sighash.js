 ...
 * @param {Transaction} transaction
 * @param {PrivateKey} signInterface
 * @param {number} sighashType the type of the hash
 * @param {number} inputNumber the input index for the signature
 * @param {Script} subscript the script that will be signedt
 * @param {String} signingMethod - method used to sign - 'ecdsa' or 'schnorr' (future signing method)
 * @return {Signature}
 */
async function signWithInterface(transaction, signInterface, sighashType, inputIndex, subscript, signingMethod) {
  signingMethod = signingMethod || 'ecdsa';

  if(signingMethod === 'ecdsa') {
    const hashbuf = sighash(transaction, sighashType, inputIndex, subscript);
    const sig = await signInterface(buffer.reverse(hashbuf))
    return sig;
  }
  throw new Error("signingMethod not supported ", signingMethod);
}
