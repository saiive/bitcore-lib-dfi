...
Transaction.prototype.signLedger = async function(getSignLedger, publicKey, sigtype, signingMethod) {
  $.checkState(this.hasAllUtxoInfo(), 'Not all utxo information is available to sign the transaction.');
  var self = this;
  if (_.isArray(getSignLedger)) {
    _.each(getSignLedger, function(getSignLedger) {
      self.signLedger(getSignLedger, publicKey, sigtype, signingMethod);
    });
    return this;
  }
  const signs = await this.getSignaturesLedger(getSignLedger, publicKey, sigtype, signingMethod)
...
Transaction.prototype.getSignaturesLedger = async function(getSignLedger, publicKey, sigtype, signingMethod) {
  sigtype = sigtype || Signature.SIGHASH_ALL;
  var transaction = this;
  var results = [];
  ...
  for (const [index, input] of this.inputs.entries()) {
    ...
    const signsInput = await input.getSignaturesLedger(transaction, getSignLedger, publicKey, index, sigtype, hashData, signingMethod);
