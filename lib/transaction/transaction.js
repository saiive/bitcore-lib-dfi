...
Transaction.prototype.toBufferWriter = function(writer, noWitness) {
  ...
  _.each(this.outputs, function(output) {
    output.toBufferWriter(writer, self.version);
  });
 ...
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.change = function(address, tokenId = DEFAULT_TOKEN_ID) {
  $.checkArgument(address, 'address is required');
  this._changeScript = Script.fromAddress(address);
  this._updateChangeOutput(tokenId);
  return this;
};
 ...
 * @return {Transaction} this, for chaining
 */
Transaction.prototype.to = function(address, amount, tokenId = DEFAULT_TOKEN_ID) {
 ...
 * @private
 */
Transaction.prototype._updateChangeOutput = function(tokenId) {
  if (!this._changeScript) {
    return;
  }
  this._clearSignatures();
...
Transaction.prototype.signWithInterface = async function(signInterface, publicKey, sigtype, signingMethod) {
  $.checkState(this.hasAllUtxoInfo(), 'Not all utxo information is available to sign the transaction.');
  var self = this;
  // Don't support multi sign
  //
  // if (_.isArray(signInterface)) {
  //   _.each(signInterface, function(signInterface) {
  //     self.signWithInterface(signInterface, publicKey, sigtype, signingMethod);
  //   });
  //   return this;
  // }
  const signs = await this.getSignaturesWithInterface(signInterface, publicKey, sigtype, signingMethod)
  _.each(signs, function(signature) {
    self.applySignature(signature, signingMethod);
  });
  return this;
};
...
Transaction.prototype.getSignaturesWithInterface = async function(signInterface, publicKey, sigtype, signingMethod) {
  sigtype = sigtype || Signature.SIGHASH_ALL;
  var transaction = this;
  var results = [];
  var hashData = Hash.sha256ripemd160(publicKey.toBuffer());
  for (const [index, input] of this.inputs.entries()) {
    const signsInput = await input.getSignaturesWithInterface(transaction, signInterface, publicKey, index, sigtype, hashData, signingMethod);
    _.each(signsInput, function(signature) {
      results.push(signature);
    });
  };
  return results;
...
Transaction.customTxMarker = 'DfTx';

Transaction.prototype.checkCustom = function() {
  var scriptBuffer = this.outputs[0]._scriptBuffer;
  var script = Script.fromBuffer(scriptBuffer);
  var br = new BufferReader(script.chunks[0].buf);
  if (!br.eof()) {
    return {
      isCustom: br.read(4).toString() === Transaction.customTxMarker,
      br: br,
    };
  }
...
Transaction.prototype.getCustom = function() {
  try {
    var checkCustom = this.checkCustom();
    if (checkCustom.isCustom) {
      var custom = {};
      custom.txType = checkCustom.br.read(1).toString();
      switch (custom.txType) {
        case customTx.customTxType.createMasternode:
          custom.data = new customTx.CreateMasternode(checkCustom.br);
          break;
        case customTx.customTxType.resignMasternode:
          custom.data = new customTx.ResignMasternode(checkCustom.br);
          break;
        case customTx.customTxType.createToken:
          custom.data = new customTx.CreateToken(checkCustom.br);
          break;
        case customTx.customTxType.mintToken:
          custom.data = new customTx.MintToken(checkCustom.br);
          break;
        case customTx.customTxType.updateToken:
          custom.data = new customTx.UpdateToken(checkCustom.br);
          break;
        case customTx.customTxType.updateTokenAny:
          custom.data = new customTx.UpdateTokenAny(checkCustom.br);
          break;
        case customTx.customTxType.createPoolPair:
          custom.data = new customTx.CreatePoolPair(checkCustom.br);
          break;
        case customTx.customTxType.updatePoolPair:
          custom.data = new customTx.UpdatePoolPair(checkCustom.br);
          break;
        case customTx.customTxType.poolSwap:
          custom.data = new customTx.PoolSwap(checkCustom.br);
          break;
        case customTx.customTxType.addPoolLiquidity:
          custom.data = new customTx.AddPoolLiquidity(checkCustom.br);
          break;
        case customTx.customTxType.removePoolLiquidity:
          custom.data = new customTx.RemovePoolLiquidity(checkCustom.br);
          break;
        case customTx.customTxType.utxosToAccount:
          custom.data = new customTx.UtxosToAccount(checkCustom.br);
          break;
        case customTx.customTxType.accountToUtxos:
          custom.data = new customTx.AccountToUtxos(checkCustom.br);
          break;
        case customTx.customTxType.accountToAccount:
          custom.data = new customTx.AccountToAccount(checkCustom.br);
          break;
        case customTx.customTxType.setGovVariable:
          custom.data = new customTx.SetGovVariable(checkCustom.br);
          break;
        case customTx.customTxType.anyAccountsToAccounts:
          custom.data = new customTx.AnyAccountsToAccounts(checkCustom.br);
          break;
        default:
          break;
      }
