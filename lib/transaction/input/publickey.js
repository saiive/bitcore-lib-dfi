...
PublicKeyInput.prototype.getSignaturesWithInterface = async function(transaction, signInterface, publicKey, index, sigtype, hashData, signingMethod) {
  $.checkState(this.output instanceof Output);
  sigtype = sigtype || Signature.SIGHASH_ALL;
  if (publicKey.toString() === this.output.script.getPublicKey().toString('hex')) {
    const signature = await Sighash.signWithInterface(transaction, signInterface, sigtype, index, this.output.script, signingMethod);
    return [new TransactionSignature({
      publicKey: publicKey,
      prevTxId: this.prevTxId,
      outputIndex: this.outputIndex,
