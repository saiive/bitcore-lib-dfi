...
MultiSigScriptHashInput.prototype.getSignaturesWithInterface = async function(transaction, signInterface, publicKeyArg, index, sigtype, hashData, signingMethod) {
  $.checkState(this.output instanceof Output);
  sigtype = sigtype || Signature.SIGHASH_ALL;
  signingMethod = signingMethod || 'ecdsa';
  ...
  var results = [];
    ...
    if (publicKey.toString() === publicKeyArg.toString()) {
      var signature;
      if (self.nestedWitness || self.type === Address.PayToWitnessScriptHash) {
        var scriptCode = self.getScriptCode();
        var satoshisBuffer = self.getSatoshisBuffer();
        signature = await SighashWitness.signWithInterface(transaction, signInterface, sigtype, index, scriptCode, satoshisBuffer, signingMethod);
      } else  {
        signature = await Sighash.signWithInterface(transaction, signInterface, sigtype, index, self.redeemScript, signingMethod);
      }
      results.push(new TransactionSignature({
