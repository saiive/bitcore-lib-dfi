...
MultiSigScriptHashInput.prototype.getSignaturesLedger = async function(transaction, getSignLedger, publicKeyArg, index, sigtype, hashData, signingMethod) {
  $.checkState(this.output instanceof Output);
  sigtype = sigtype || Signature.SIGHASH_ALL;
  signingMethod = signingMethod || 'ecdsa';
  ...
  var results = [];
    ...
    if (publicKey.toString() === publicKeyArg.toString()) {
      ...
      if (self.nestedWitness || self.type === Address.PayToWitnessScriptHash) {
        var scriptCode = self.getScriptCode();
        var satoshisBuffer = self.getSatoshisBuffer();
        signature = await SighashWitness.signLedger(transaction, getSignLedger, sigtype, index, scriptCode, satoshisBuffer, signingMethod);
      } else  {
        signature = await Sighash.signLedger(transaction, getSignLedger, sigtype, index, self.redeemScript, signingMethod);
      }
      results.push(new TransactionSignature({
