...
MultiSigInput.prototype.getSignaturesLedger = async function(transaction, getSignLedger, publicKeyArg, index, sigtype, hashData, signingMethod) {
  $.checkState(this.output instanceof Output);
  sigtype = sigtype || Signature.SIGHASH_ALL;
  signingMethod = signingMethod || 'ecdsa';
  ...
  for (const publicKey of this.publicKeys) {
    if (publicKey.toString() === publicKeyArg.toString()) {
      const signature = await Sighash.signLedger(transaction, getSignLedger, sigtype, index, self.output.script, signingMethod);
      results.push(new TransactionSignature({
