...
PublicKeyHashInput.prototype.getSignaturesWithInterface = async function(transaction, signInterface, publicKey, index, sigtype, hashData, signingMethod) {
  $.checkState(this.output instanceof Output);
  ...
  if (script && BufferUtil.equals(hashData, script.getPublicKeyHash())) {
    ...
    if (script.isWitnessPublicKeyHashOut()) {
      var satoshisBuffer = this.getSatoshisBuffer();
      var scriptCode = this.getScriptCode(publicKey);
      ...
      signature = await Sighash.signWithInterface(transaction, signInterface, sigtype, index, this.output.script, signingMethod);
    }
