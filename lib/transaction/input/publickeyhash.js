...
PublicKeyHashInput.prototype.getSignaturesLedger = async function(transaction, getSignLedger, publicKey, index, sigtype, hashData, signingMethod) {
  $.checkState(this.output instanceof Output);
  ...
  if (script && BufferUtil.equals(hashData, script.getPublicKeyHash())) {
    ...
    if (script.isWitnessPublicKeyHashOut()) {
      ...
      signature = await Sighash.signLedger(transaction, getSignLedger, sigtype, index, this.output.script, signingMethod);
    }
