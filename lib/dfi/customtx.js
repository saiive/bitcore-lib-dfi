...
CreatePoolPair.fromBuffer = function(br) {
  var data = {};
  data.idTokenA = br.readUInt32LE().toString('hex');
  data.idTokenB = br.readUInt32LE().toString('hex');
...
UpdatePoolPair.fromBuffer = function(br) {
  var data = {};
  data.pollId = br.readUInt32LE().toString('hex');
  data.status = br.readUInt8();
...
PoolSwap.fromBuffer = function(br) {
  ...
  data.idTokenFrom = br.readUInt32LE().toString('hex');
  data.idTokenTo = br.readUInt32LE().toString('hex');
  data.amountFrom = br.readUInt64LEBN().toNumber();
  data.maxPrice = {
    integer: br.readUInt64LEBN().toNumber(),
    fraction: br.readUInt64LEBN().toNumber(),
  };
  return data;
};
...
RemovePoolLiquidity.fromBuffer = function(br) {
  var data = {};
  data.from = CScript.fromBuffer(br);
  data.nTokenId = br.readUInt32LE().toString('hex');
  data.nValue = br.readVarintNum();
  return data;
};
...
var AnyAccountsToAccounts = function AnyAccountsToAccounts(arg) {
...
AnyAccountsToAccounts.fromBuffer = function(br) {
  var data = {};
  data.from = new CAccounts(br);
  data.to = new CAccounts(br);
  return data;
};
