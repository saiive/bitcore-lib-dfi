...
CreateMasternode.toBuffer = function (data) {
  ...
  bw.writeUInt8(CUSTOM_SIGNATURE);
  bw.writeUInt8(customTxType.createMasternode);
  bw.writeUInt8(data.operatorType);
  bw.write(data.operatorAuthAddress);
};
...
ResignMasternode.toBuffer = function (data) {
  ...
  var bw = new BufferWriter();
  bw.writeUInt8(CUSTOM_SIGNATURE);
  bw.writeUInt8(customTxType.resignMasternode);
  bw.writeUInt32LE(data.nodeId);
};
